...
export function merge(
  ...
  for (let name of right_join) {
    let a = after.filter(x => x.name == name)[0]
    listOfNodes.push(
      new After(a.name, a.latestVersion(), mapDependencyToAfterTree(a.children))
    )
  }
...
function mapDependencyToBeforeTree(
  ...
  for (let x of dependencies) {
    list.push(
      new Before(x.name, x.version_info, mapDependencyToBeforeTree(x.children))
    )
  }
  return list
}

function mapDependencyToAfterTree(
  dependencies: Dependency[]
): DependencyTree[] {
  if (dependencies.length == 0) {
    return []
  ...
  for (let x of dependencies) {
    list.push(
      new After(x.name, x.version_info, mapDependencyToAfterTree(x.children))
    )
  }
  return list
}
